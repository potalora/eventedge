...
class CohortOrchestrator:
    ...
    def run_daily(self, trading_date: str | None = None) -> dict[str, Any]:
        """Coordinate the ordered, behavior-preserving daily pipeline phases."""
        from tradingagents.strategies.orchestration.daily_pipeline import (
            DailyRunState,
            partition_daily_replay,
            run_candidate_reference_validation,
            run_candidate_volatility_validation,
            run_governed_execution,
            run_horizon_screening,
            stage_daily_results,
        )
        ...
        if pending_gap is not None:
            ...
            if session == pending_gap.gap_session:
                return state.finalize(recovered)

        # The historical empty-roster result intentionally bypasses finalization.
        if not self.cohorts:
            return results

        metric_epoch = self.cohorts[0]["executor"].ensure_metric_epoch(
            self._metric_epoch_context,
            session,
        )
        ...
        for phase in phases:
            phase_result = phase(state)
            if phase_result is not None:
                return phase_result
        return stage_daily_results(state)
