...
An autonomous event-driven trading research system that uses LLMs to analyze stocks and run paper trading strategies. It's a personal project — not a product, not a service, not financial advice.
...
The original TauricResearch core pipeline code is Apache 2.0 (see `LICENSE-APACHE`). All other code is proprietary (see `LICENSE` and `NOTICE`).
