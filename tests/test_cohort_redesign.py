...
class TestPortfolioSizeProfile:
    ...
    def test_larger_portfolios_have_stricter_concentration(self):
        ...
        assert (
            SIZE_PROFILES["5k"].max_position_pct
            > SIZE_PROFILES["100k"].max_position_pct
        )
        assert (
            SIZE_PROFILES["5k"].sector_concentration_cap
            > SIZE_PROFILES["100k"].sector_concentration_cap
        )
...
class TestHorizonParams:
    ...
    def test_longer_horizons_have_longer_hold_days(self):
        ...
        assert (
            HORIZON_PARAMS["30d"]["hold_days_default"]
            < HORIZON_PARAMS["3m"]["hold_days_default"]
        )
        assert (
            HORIZON_PARAMS["3m"]["hold_days_default"]
            < HORIZON_PARAMS["6m"]["hold_days_default"]
        )
        assert (
            HORIZON_PARAMS["6m"]["hold_days_default"]
            < HORIZON_PARAMS["1y"]["hold_days_default"]
        )
    ...
    def test_hold_days_range_contains_default(self):
        ...
        for horizon, params in HORIZON_PARAMS.items():
            ...
            assert lo <= default <= hi, (
                f"{horizon}: default {default} not in range ({lo}, {hi})"
            )
...
class TestBuildDefaultCohorts:
    ...
    def test_custom_base_state_dir(self):
        ...
        cohorts = build_default_cohorts(
            {"autoresearch": {"state_dir": "/tmp/test_state"}}
        )
        for c in cohorts:
...
class TestRiskGateCashReserve:
    ...
    def test_cash_reserve_blocks_when_insufficient(self):
        gate = self._make_gate(
            cash_reserve_pct=0.15, cash=9000.0, portfolio_value=10000.0
        )
    ...
    def test_zero_cash_reserve_always_passes(self):
        gate = self._make_gate(
            cash_reserve_pct=0.0, cash=1000.0, portfolio_value=10000.0
        )
        ...
        for s in strategies:
            for h in ["30d", "3m", "6m", "1y"]:
                ...
                assert isinstance(params, dict), (
                    f"{s.name} get_default_params(horizon={h}) not dict"
                )
        ...
        for s in strategies:
            for h in ["30d", "3m", "6m", "1y"]:
                ...
                assert isinstance(space, dict), (
                    f"{s.name} get_param_space(horizon={h}) not dict"
                )
...
class TestOrchestratorHorizonScreening:
    ...
    def test_screen_for_horizon_uses_horizon_params(self):
        ...
        with patch(
            ...
            with patch(
                "tradingagents.strategies.orchestration.multi_strategy_engine.build_default_registry"
            ):
...
class TestCohortComparisonMatrix:
    """Test the report-only v2 heatmap adapter."""
    ...
    def comparison(self):
        ...
        return CohortComparison(metrics_service=service)
    ...
    def test_heatmap_returns_matrix(self, comparison):
        result = comparison.heatmap(metric="total_return")
        assert result["30d"]["5k"] == 0.01
        assert result["3m"]["10k"] == 0.01
        assert result["1y"]["100k"] is None
...
class TestIntegration:
    ...
    def test_build_cohorts_and_comparison_roundtrip(self, tmp_path):
        ...
        assert len(cohorts) == 16
        assert len({cohort.name for cohort in cohorts}) == 16
