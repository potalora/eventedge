...
class TestCFTCSource:
    ...
    def test_cftc_source_unavailable(self):
        ...
        with patch.dict("sys.modules", {"cot_reports": None}):
            with patch(
                "builtins.__import__", side_effect=ImportError("no cot_reports")
            ):
...
class TestCommodityMacroStrategy:
    ...
    def _make_cot_data(
        self, gold_pctl=0.5, crude_pctl=0.5, gold_dir="neutral", crude_dir="neutral"
    ):
        result = {
            "gold": {
                "net_position": 100_000,
                "percentile": gold_pctl,
                "wow_change": 0,
                "direction_signal": gold_dir,
            },
            "crude_oil": {
                "net_position": -50_000,
                "percentile": crude_pctl,
                "wow_change": 0,
                "direction_signal": crude_dir,
            },
            "silver": {
                "net_position": 50_000,
                "percentile": 0.5,
        ...
        for commodity, values in result.items():
            values["report_id"] = f"CFTC:{commodity}:2026-03-27"
            values["window_end"] = "2026-03-27"
        return result

    def _make_fred_data(
        self, fed_funds=5.0, cpi_latest=3.0, cpi_3m_ago=3.0, vix=20.0, yield_curve=0.5
    ):
        return {
            "FEDFUNDS": {pd.Timestamp("2026-01-01"): fed_funds},
            "CPIAUCSL": {
                pd.Timestamp("2025-10-01"): cpi_3m_ago,
                pd.Timestamp("2026-01-01"): cpi_latest,
            },
    ...
    def test_screen_catalyst_boost(self):
        ...
        catalyst_data["regulations"] = {
            "results": [{"title": "New gold mining regulation announced"}]
        }
    ...
    def test_short_only_enforcement(self):
        ...
        candidates = strategy.screen(data, "2026-04-01", params)
        uso_longs = [
            c for c in candidates if c.ticker == "USO" and c.direction == "long"
        ]
        assert len(uso_longs) == 0
        xle_longs = [
            c for c in candidates if c.ticker == "XLE" and c.direction == "long"
        ]
    ...
    def test_screen_works_with_default_params(self):
        ...
        candidates = strategy.screen(data, "2026-04-01", params)
        assert len(candidates) > 0, (
            "screen() should produce candidates with default params"
        )
...
class TestPortfolioCommittee:
    ...
    def test_commodity_regime_alignment_crisis_gld(self):
        ...
        result = committee._assess_regime_alignment(
            "long", {"overall_regime": "crisis"}, ticker="GLD"
        )
    ...
    def test_commodity_regime_alignment_crisis_xle(self):
        ...
        committee = PortfolioCommittee()
        result = committee._assess_regime_alignment(
            "long", {"overall_regime": "crisis"}, ticker="XLE"
        )
    ...
    def test_commodity_regime_alignment_stressed_slv(self):
        ...
        committee = PortfolioCommittee()
        result = committee._assess_regime_alignment(
            "long", {"overall_regime": "stressed"}, ticker="SLV"
        )
        assert result == "aligned"
