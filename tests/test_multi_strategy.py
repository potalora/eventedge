...
class TestMultiStrategyEngine:
    ...
    def test_engine_has_paper_trade_strategies(self, engine):
        assert len(engine.paper_trade_strategies) == 7
...
class TestPortfolioCommittee:
    ...
    def test_single_strategy_needs_high_confidence(self):
        ...
        result = pc.synthesize(signals=signals, strategy_confidence={"earnings": 0.3})
        assert len(result) == 0

        # High confidence -> included
        result = pc.synthesize(signals=signals, strategy_confidence={"earnings": 0.9})
        assert len(result) >= 1
