...
- durable hydration remains exact-session and exact-epoch scoped;
...
1. Record the enabled/active state of `trade.timer`, `trade-rerun.path`, and `trade-preflight.timer`. Stop and mask those three entry points plus `trade.service`, `trade-rerun.service`, and `trade-preflight.service` for the mutation window. Confirm all three services are inactive, no daily/preflight worker remains, and `.triggers/run-now` is absent.
...
3. Make and verify a timestamped rollback archive of the entire `gen_011` state and relevant manifest material before changing ledger state. Record the archive hash and prove a test extraction reproduces the captured database hashes.
...
9. Run generation/status and no-write preflight smoke checks, verify the manager supplies the `gen_012` ID, reviewed commit, and isolated state path to its worktree, then unmask all six units and restore the three entry points to their recorded enabled state. Confirm the next triggers. Do not invoke a historical replay.
